...
const { catalogApi, locationsApi, inventoryApi } = new Client(config)

const addInventory = async (changes) => {
...
const addImages = async (image, catalogObjectId, success) => {
  ...
  const directoryPath = path.join(__dirname, `Images/${image.folder}`)
  ...
  //listing all files using forEach
   const images = files.map((file) => {
          ...
          return {
            url: path.join(__dirname, `Images/${image.folder}/`, file),
            request: {
              idempotencyKey: uuidv4(),
  ...
  for (let i=0; i < images.length; i++){
    const fileReadStream = fs.createReadStream(images[i].url)
    const imageFile = new FileWrapper(fileReadStream, {
      contentType: 'image/jpeg',
    })
  
    try {
      const { result: { image } } = await catalogApi.createCatalogImage(images[i].request,imageFile)
      success()
 ...
 */
const getCurrency = async (locationResponse) => {
...
const addItems = async () => {
  ...
  const locationId = locationResponse.result?.location?.id

  const currency = await getCurrency(locationResponse)
  ...
  if (currency) {
    ...
    // Create batch inventory request
    const { result: { objects } } = await catalogApi.searchCatalogObjects({ objectTypes:['ITEM_VARIATION'] })
    ...
    try{
      addInventory(changes)
    } catch(error) {
      console.error('updating object inventory failed: ', error)
    }
...
const clearCatalog = async () => {
  try {
    const { result: { objects } } = await catalogApi.listCatalog(undefined, 'ITEM,ITEM_VARIATION,TAX,IMAGE,CATEGORY,ITEM_OPTION,ITEM_OPTION_VAL')
